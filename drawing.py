...
class Drawing:
    ...
    def background(self):
        ...
        rectangle2 = Rectangle(pos = [NULLX,HALF_HEIGHT+NULLY], size = [WIDTH, HALF_HEIGHT])
    ...
    def mini_map(self, player):
        ...
        self.sc.add(Rectangle(pos=[NULLX+int(map_x)-3, NULLY+int(map_y)-3], size=[6,6]))
        

        self.sc.add(Color(0,.8,0,.5))
        for x, y in mini_map:
            self.sc.add(Rectangle(pos=[NULLX+x,NULLY+y], size=[MAP_TILE, MAP_TILE]))
            
    def sight(self):
        self.sc.add(Color(1,0,0,.5))
        self.sc.add(Rectangle(pos=[NULLX+HALF_WIDTH-5, NULLY+HALF_HEIGHT-5], size=(10,10)))
